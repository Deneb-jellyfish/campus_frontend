      ...
      >
        ...
        <view v-else class="message-row self">
          <view class="message-bubble self">
            <!-- 发送状态 -->
            <view v-if="msg.status === 'sending'" class="sending-icon">⏳</view>
            <view v-else-if="msg.status === 'failed'" class="failed-icon" @click="resendMessage(msg)">⚠️</view>
